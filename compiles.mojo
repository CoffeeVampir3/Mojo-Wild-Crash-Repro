            ...
            for n_begin in range(n_offset, n_offset + n_size, N_STEP):
                ...
                if is_last_k_block:
                    ...
                    for m_local in range(M_STEP):
                        ...
                        for n_local in range(N_STEP):
                            var n = n_begin + n_local
                            var scale_b = B.scales[0, n][0]
                            var acc_val = acc_buf.offset(m_local * N_STEP + n_local).load[width=1]()
                            var scaled = (acc_val.cast[DType.float32]() * scale_a * scale_b).cast[DType.int32]()
